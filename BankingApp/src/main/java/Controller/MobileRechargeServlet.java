...
public class MobileRechargeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			double currentBalance = rs.getDouble("ACCBAL");
			...
			if (currentBalance >= amount && currentBalance - amount >= 1000) {
				...
				if (updateStatus == 1) {
					...
					insertTransactionStmt.setLong(1, mobileNumber);
					...
					if (transactionStatus == 1) {
						...
						out.println("<h2>Recharge Successful!</h2>");
						...
						response.sendRedirect("'RechargeSuccess.html'");
					} else {
				...
				response.getWriter().println("<a href='mobileRecharge.jsp'>Back</a>");
			}
